...
def test_github_runtime_cannot_reenable_generic_adaptive() -> None:
    root = Path(__file__).resolve().parents[1]
    source = (root / "scripts" / "github_common.py").read_text(
        encoding="utf-8"
    )

    assert 'values.setdefault("adaptive", {})["enabled"] = False' in source
    assert "aggressive_paper_mode" not in source
    assert "AGGRESSIVE_STRUCTURAL_RISK_SCALED" in source
