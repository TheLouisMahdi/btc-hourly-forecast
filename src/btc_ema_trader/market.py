...
class MarketDataClient:
    def __init__(self, settings: Settings):
        ...
        self.session.headers.update(
            {"User-Agent": "btc-structural-breakout/4.0"}
        )
    ...
    def fetch_history(
        ...
        days = float(days or self.cfg.get("history_days", 365))
    ...
    def _fetch_okx(self, days: float) -> pd.DataFrame:
        ...
        maximum_pages = max(80, required_pages)
    ...
    def _get_json(
        ...
        timeout = float(
            self.cfg.get("request_timeout_seconds", 20)
        )
        backoff = float(
    ...
    def _validate(
        ...
        if maximum_gap > float(
            self.cfg.get("maximum_gap_hours", 2)
        ):
            raise ValueError(
        ...
        configured_minimum = int(
            self.cfg.get("minimum_history_rows", 8000)
        )
        minimum = (
            min(expected, configured_minimum)
            if days >= 150
            else expected
        )
