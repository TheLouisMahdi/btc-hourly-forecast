...
def make_decision(
    ...
    trade_direction = str(prediction.get("trade_direction", forecast_direction))
    selected_horizon = int(prediction["selected_horizon"])
    continuation = prediction.get("continuation", {})
    confidence = float(continuation.get(selected_horizon, continuation.get(str(selected_horizon), 0.5)))
    event_agreement = float(prediction.get("event_agreement", 0.0))
    event_aligned_return = float(prediction.get("expected_event_aligned_return", 0.0))
    expected_return = float(prediction.get("expected_return", 0.0))
    tradeability_probability = float(prediction["tradeability"][selected_horizon])
    costs = execution_cost_breakdown(cfg)
    net_edge_bps = event_aligned_return * 10_000 - costs["stress_cost_bps"]
    blockers: list[str] = []

    qualified_horizons = {int(h) for h in qualification.get("qualified_horizons", [])}
    ...
    if not is_event or event_direction == 0:
        blockers.append("NO_NEW_MARKET_EVENT")
    ...
    # Trade direction is fixed by the primary event. The ML layer only accepts/rejects it.
    expected_trade_direction = "UP" if event_direction > 0 else "DOWN" if event_direction < 0 else trade_direction
    if is_event and trade_direction != expected_trade_direction:
        blockers.append("EVENT_DIRECTION_MISMATCH")

    min_confidence = _per_horizon(cfg.get("minimum_confidence", {}), selected_horizon, 0.57)
    if confidence < min_confidence:
        blockers.append("LOW_CONTINUATION_PROBABILITY")
    min_tradeability = _per_horizon(
        cfg.get("minimum_tradeability_probability", {}), selected_horizon, 0.56
    )
    if tradeability_probability < min_tradeability:
        blockers.append("LOW_TRADEABILITY_PROBABILITY")
    if event_agreement < float(cfg.get("minimum_horizon_agreement", 2 / 3)):
        blockers.append("CONTINUATION_HORIZON_DISAGREEMENT")
    ...
    elif atr_pct * 100 > float(cfg.get("maximum_atr_percent", 3.0)):
        blockers.append("VOLATILITY_SHOCK")
    if int(latest_row.get("news_shock", 0)) == 1 and bool(cfg.get("block_during_news_shock", True)):
    ...
    cooldown = float(cfg.get("cooldown_hours_after_signal", 2))
    if hours_since_last_signal is not None and hours_since_last_signal < cooldown:
    ...
    if data_health:
        ...
        if data_health.get("news_stale", False) and bool(cfg.get("require_fresh_news", False)):
            blockers.append("NEWS_STALE")

    action = "LONG" if expected_trade_direction == "UP" else "SHORT"
    if blockers:
        action = "WAIT"
    trade_plan = build_trade_plan(latest_row, expected_trade_direction, prediction, settings, action)
    return Decision(
        ...
        probabilities={int(k): float(v) for k, v in prediction["probabilities"].items()},
        tradeability={int(k): float(v) for k, v in prediction["tradeability"].items()},
        returns={
            int(k): float(v)
            for k, v in prediction.get("absolute_event_returns", prediction.get("returns", {})).items()
        },
        blockers=blockers,
...
def build_trade_plan(
    ...
    atr = float(row.get("atr", price * float(cfg.get("minimum_stop_percent", 0.0045))))
    raw_stop_pct = float(cfg.get("stop_atr_multiplier", 1.15)) * atr / price
    stop_pct = float(
        np.clip(
            raw_stop_pct,
    ...
    selected_horizon = int(prediction["selected_horizon"])
    predicted_move = abs(float(prediction.get("expected_event_aligned_return", 0.0)))
    target_pct = max(float(cfg.get("target_r_multiple", 1.7)) * stop_pct, predicted_move)
    if direction == "UP":
    ...
    gap_buffer = float(cfg.get("gap_risk_buffer_bps", 6.0)) / 10_000
    ...
    leverage = min(float(cfg.get("maximum_leverage", 3.0)), max(1.0, notional / max(account, 1e-9)))
    return {
        "status": "ACTIONABLE" if action in {"LONG", "SHORT"} else "BLOCKED",
        "event_id": row.get("event_id"),
        "event_type": row.get("event_type", "NONE"),
        "regime": row.get("regime", "UNKNOWN"),
        "trade_direction_source": "PRIMARY_EVENT",
