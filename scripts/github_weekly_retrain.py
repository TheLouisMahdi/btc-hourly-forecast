...
from btc_ema_trader.training import train_from_database
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Retrain the model from fresh 180-day data")
...
def main() -> int:
    ...
    settings = build_github_settings(root, runtime_dir, model_dir=model_dir, report_dir=report_dir)
    configure_logging(settings, verbose=True)
    database = Database(settings)
    database.initialize()

    started = pd.Timestamp.now(tz="UTC")
    market = fetch_and_store(settings, database, days=180, provider=None)
    historical_news = optional(lambda: collect_and_store(settings, database, historical=True, days=180))
    recent_news = optional(lambda: collect_and_store(settings, database, historical=False))
    training = train_from_database(settings, database, provider=market["provider"])
    ...
    for filename in ("latest_training_report.json", "latest_metrics.csv"):
